...
import styles from "../../../(dashboard)/penarikan/page.module.css";

export default function ShopPenarikanPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (store) {
        ...
        if (txs) {
          // Filter store withdrawals
          const storeWithdrawals = txs.filter(t => t.details?.is_store_withdrawal === true);
          setHistory(storeWithdrawals);
        }
      }
    } catch (err) {
      console.error(err);
  ...
  const handleSubmit = async () => {
    if (!supabaseUser || !storeId) return;
    ...
    if (amount > balance) {
      alert("Saldo pendapatan Toko Anda tidak mencukupi untuk penarikan ini.");
      return;
    }
    if (!agreed) {
  ...
  return (
    <div className={styles.container} style={{ minHeight: "auto", padding: "24px" }}>
      <header className={styles.header}>
        <h1 className={styles.title}>Penarikan Saldo Toko</h1>
        <p className={styles.subtitle}>Tarik pendapatan hasil penjualan produk Anda</p>
      </header>
      ...
      {activeTab === "Minta Penarikan" && (
        <div className={styles.formLayout}>
          ...
          <div className={styles.formSection}>
            ...
            <div className={styles.formGroup}>
              <label className={styles.formLabel}>Jumlah (Min. Rp1)</label>
              ...
              {amount > 0 && (
                <div style={{ marginTop: 8, padding: 12, backgroundColor: "var(--bg-main)", borderRadius: 8, fontSize: 13, border: "1px solid var(--border-color)", display: "flex", flexDirection: "column", gap: 4 }}>
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span style={{ color: "var(--text-muted)" }}>Potongan Admin (5%):</span>
                    <span style={{ fontWeight: 600, color: "var(--danger)" }}>-{formatPrice(Math.round(amount * 0.05))}</span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px dashed var(--border-color)", paddingTop: 4, marginTop: 4 }}>
                    <span style={{ fontWeight: "bold", color: "var(--text-main)" }}>Diterima Bersih:</span>
                    <span style={{ fontWeight: "bold", color: "var(--primary)" }}>{formatPrice(amount - Math.round(amount * 0.05))}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className={styles.checkboxGroup}>
              ...
              />
              <label htmlFor="agree-shop" className={styles.checkboxLabel}>
            ...
            <button
              ...
              disabled={isSubmitting || !agreed || amount <= 0 || amount > balance}
              style={{
                opacity: (isSubmitting || !agreed || amount <= 0 || amount > balance) ? 0.5 : 1,
                cursor: (isSubmitting || !agreed || amount <= 0 || amount > balance) ? "not-allowed" : "pointer",
              }}
            >
          ...
          <div className={styles.saldoCard}>
            ...
            <h3 className={styles.saldoTitle}>Total Pendapatan Anda</h3>
            <p className={styles.saldoDesc}>
              Ini adalah total pendapatan bersih dari toko Anda yang dapat ditarik ke rekening.
            </p>
            {totalWithdrawn > 0 && (
                  ...
                  {history.map(t => {
                    ...
                    return (
                      <tr key={t.id} style={{ borderBottom: "1px solid var(--border-color)" }}>
                        ...
                        <td style={{ padding: 16 }}>
                          <div style={{ fontWeight: "600", color: "var(--danger)" }}>-{formatPrice(t.amount)}</div>
                          {t.details?.receive_amount && (
