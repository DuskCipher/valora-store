...
async function check() {
  ...
  const { data: products } = await supabase.from('products').select('id, name, store_id, price');
  console.log('Products:', products);

  const { data: txs } = await supabase.from('transactions').select('*').eq('type', 'order').order('created_at', { ascending: false });
  console.log('Orders:', JSON.stringify(txs, null, 2));

  fs.writeFileSync('db_dump.json', JSON.stringify({ stores, products, txs }, null, 2));
}
